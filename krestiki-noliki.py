...
def on_closing():
    global app_running
    if messagebox.askokcancel("Дать заднюю", "Уже наигрались?"):
        app_running = False
        tk.destroy()
...
b1 = Button(tk, text="Начать с начала!", command=button_press)
b1.pack()
...
def check_winner(who):
    ...
    win = True
    for i in range(0, s_y):
        print(points[s_y - 1 - i][i])
        if points[s_y - 1 - i][i] != who:
            win = False
    if win:
        return True
